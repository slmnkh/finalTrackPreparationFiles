...
def get_features(filePath, **args):
    ...
    minf = min(features)
    maxf = max(features)
    if features.index(maxf) < features.index(minf):
        order_flag = 0
    else:
        order_flag = 1
    features = [minf, maxf, order_flag]
    ...
    if return_string_flag is 0:
        ...
        return convert_to_svm_input(features), headings, reduced_track
